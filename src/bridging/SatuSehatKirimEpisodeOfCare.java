                ...
                try {
                    ...
                    try {
                        ...
                        if (!response.asText().equals("")) {
                            Sequel.menyimpan("satu_sehat_episode_of_care","null,?,?,?,?","EpisodeOfCare",4,new String[]{
    ...
    private void tampil() {
        ...
        try {
            String queryRalan = "select reg_periksa.tgl_registrasi,reg_periksa.jam_reg,reg_periksa.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,pasien.no_ktp,"
                    + "reg_periksa.stts,reg_periksa.status_lanjut,concat(replace(pemeriksaan_ralan.waktu_pulang,' ','T'),'+07:00') as pulang,satu_sehat_encounter.id_encounter, "
                    ...
                    + "inner join (select no_rawat,max(concat(tgl_perawatan,' ',jam_rawat)) as waktu_pulang from pemeriksaan_ralan "
                    + "where tgl_perawatan between ? and ? group by no_rawat) as pemeriksaan_ralan on pemeriksaan_ralan.no_rawat=reg_periksa.no_rawat "
                    + "inner join (select no_rawat,max(id_encounter) as id_encounter from satu_sehat_encounter group by no_rawat) as satu_sehat_encounter "
                    + "on satu_sehat_encounter.no_rawat=reg_periksa.no_rawat inner join "
                    + "(select distinct no_rawat,kd_penyakit,status from diagnosa_pasien) as diagnosa_pasien on diagnosa_pasien.no_rawat=reg_periksa.no_rawat "
                    + "inner join penyakit on diagnosa_pasien.kd_penyakit=penyakit.kd_penyakit left join "
                    + "(select no_rawat,kd_penyakit,status,max(id_episode_of_care) as id_episode_of_care from satu_sehat_episode_of_care "
                    + "group by no_rawat,kd_penyakit,status) as satu_sehat_episode_of_care on satu_sehat_episode_of_care.no_rawat=diagnosa_pasien.no_rawat "
            ...
            String queryRanap = "select reg_periksa.tgl_registrasi,reg_periksa.jam_reg,reg_periksa.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,pasien.no_ktp,"
                    + "reg_periksa.stts,reg_periksa.status_lanjut,concat(replace(kamar_inap.waktu_pulang,' ','T'),'+07:00') as pulang,satu_sehat_encounter.id_encounter, "
                    + "diagnosa_pasien.kd_penyakit,penyakit.nm_penyakit,"
                    + "case "
                    + "when diagnosa_pasien.kd_penyakit like 'O%' then 'ANC' "
                    + "when left(diagnosa_pasien.kd_penyakit,3) in ('A15','A16','A17','A18','A19') then 'TB-SO' "
                    ...
                    + "ifnull(satu_sehat_episode_of_care.id_episode_of_care,'') as id_episode_of_care "
                    + "from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                    + "inner join (select no_rawat,max(concat(tgl_keluar,' ',jam_keluar)) as waktu_pulang from kamar_inap "
                    + "where tgl_keluar between ? and ? group by no_rawat) as kamar_inap on kamar_inap.no_rawat=reg_periksa.no_rawat "
                    + "inner join (select no_rawat,max(id_encounter) as id_encounter from satu_sehat_encounter group by no_rawat) as satu_sehat_encounter "
                    + "on satu_sehat_encounter.no_rawat=reg_periksa.no_rawat inner join "
                    + "(select distinct no_rawat,kd_penyakit,status from diagnosa_pasien) as diagnosa_pasien on diagnosa_pasien.no_rawat=reg_periksa.no_rawat "
                    + "inner join penyakit on diagnosa_pasien.kd_penyakit=penyakit.kd_penyakit left join "
